...
test('reordering and conflicting binds persist independently for each healer', () => {
  ...
  settings.move('druid', 'flash', 'prayer'); settings.bind('druid', 'prayer', 'shift+q');
  ...
  assert.deepEqual(restored.spells('druid').map(s => s.id), ['greater', 'prayer', 'flash', 'penance']);
  assert.equal(restored.spells('druid').find(s => s.id === 'prayer').key, 'Shift+Q');
...
test('blocked storage retains usable session settings and damaged JSON falls back to defaults', () => {
  ...
  settings.bind('druid', 'flash', 'E'); settings.move('druid', 'penance', 'flash');
  assert.equal(settings.spells('druid')[0].id, 'penance');
  assert.equal(settings.spells('druid').find(s => s.id === 'flash').key, 'E');
