...
function allocations(healerId, chapterIndex, earnedFirstPoint, variant) {
...
function equipExpected(equipment, healerId) {
...
function chooseCast(game, healerId) {
  ...
  const injured = living.filter(member => missing(member) > 55);
  ...
  if (healerId === 'priest') {
    ...
    if (missing(lowest) >= 100 && can('penance') && game.begin('penance', lowest.id).ok) return;
    ...
    if (game.mana > game.maxMana * .8 && can('holyFire')) game.begin('holyFire', 'boss');
  } else {
    if (injured.length >= 3 && injured.reduce((sum, member) => sum + Math.min(150, missing(member)), 0) >= 320 && can('tranquility') && game.begin('tranquility', lowest.id).ok) return;
    if (injured.length >= 3 && injured.reduce((sum, member) => sum + Math.min(100, missing(member)), 0) >= 270 && can('wildGrowth') && game.begin('wildGrowth', lowest.id).ok) return;
    if (missing(lowest) >= 115 && can('swiftmend') && game.activeHots(lowest, ['rejuvenation', 'regrowth', 'wildGrowth']).length && game.begin('swiftmend', lowest.id).ok) return;
    if (missing(lowest) >= 180 && !game.activeHots(lowest, ['regrowth']).length && game.begin('regrowth', lowest.id).ok) return;
    const tank = game.party[0];
    const maxRejuv = game.spells.find(spell => spell.id === 'rejuvenation')?.hot?.maxInstances || 1;
    if (missing(tank) >= (maxRejuv > 1 && game.activeHots(tank, ['rejuvenation']).length ? 240 : 100) && game.activeHots(tank, ['rejuvenation']).length < maxRejuv && game.begin('rejuvenation', tank.id).ok) return;
    const hot = game.activeHots(lowest, ['rejuvenation']).length;
    if (missing(lowest) >= (hot ? 240 : 130) && hot < maxRejuv && game.begin('rejuvenation', lowest.id).ok) return;
    if (missing(lowest) >= 115) game.begin('nourish', lowest.id);
  }
}
function fight(encounter, equipment, healerId, random, resources, talentBuild) {
  ...
  while (game.status === 'running') {
    if (game.time >= nextDecision) { chooseCast(game, healerId); nextDecision = game.time + .12; }
    game.step();
...
function acquireRoute(chapter, route, equipment, healerId, random) {
...
function trial(chapterIndex, healerId, clearCount, seed, variant) {
  ...
  for (const [nodeIndex, node] of attemptRoute.entries()) {
    ...
    const game = fight(CHAPTER_ENCOUNTERS[node.encounter], equipment, healerId, random, resources,
      allocations(healerId, chapterIndex, earnedFirstPoint, variant));
  ...
  const boss = fight(bossEncounter, equipment, healerId, random, resources,
    allocations(healerId, chapterIndex, true, variant));
...
for (let chapterIndex = 0; chapterIndex < CHAPTERS.length; chapterIndex++) if (selectedChapters.includes(chapterIndex + 1)) for (const healerId of selectedHealers) for (const variant of chapterIndex === 3
  ...
  console.log(JSON.stringify({ chapter: chapterIndex + 1, healer: healerId, variant, clears, samples,
